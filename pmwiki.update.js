...
<VirtualHost *:81>
  ...
  // TODO: process json files containing extras
  // always process the config json first
  var installExtras = function(path, name) {

    var glob = require('glob');

    // options is optional
    var files = glob.GlobSync('./cookbooks/*.json');

      console.log('inside of glob');
      console.log(files.found);

    // grab all .json files in target directory
    // look for a local_config.json file - process first
    // TODO: down the road, files can have dependencies?
    // parse the json
    // copy source=>destination if exists and not zip
    // unzip source=> destination if exists and is zipped,
    // append to config if exists
